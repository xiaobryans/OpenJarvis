...
|---|------|-----------------------|---------------------|
| 1 | Slack — obtain `xoxp-` User OAuth Token | Yes (DM sync / full Slack connector) | No |
...
## Item 1 — Slack: `xoxp-` User OAuth Token Required

**What Bryan must do:**  
The current `JARVIS_SLACK_BOT_TOKEN` in `~/.openjarvis/cloud-keys.env` is an `xoxb-` bot token.  
Bot tokens **cannot read user DMs** (Slack platform constraint). The `slack_connector.py` explicitly rejects bot tokens.  
For full Slack sync (DM access, `#channels-history`, group DMs), Bryan must create a **Slack app with User Token Scopes** and obtain an `xoxp-` token.

**Where to do it:**  
1. Go to [https://api.slack.com/apps](https://api.slack.com/apps) — Sign in with the Jarvis HQ workspace account.
2. Create a new app → "From scratch" → name it "Jarvis Personal" → workspace: Jarvis HQ.
3. Under "OAuth & Permissions" → "User Token Scopes" — add:
   ...
   - `users:read`
4. Click "Install App to Workspace" → authorize.
...
Add line:
```
OPENCLAW_SLACK_BOT_TOKEN=xoxp-...
```
(The connector also reads `JARVIS_SLACK_BOT_TOKEN` as fallback — either key works.)
...
```
Also: `python3 -c "from openjarvis.connectors.slack_connector import SlackConnector; c = SlackConnector(); print(c.status())"` should return `status: connected`.

**Current live bot token status:**  
The `xoxb-` bot token IS live (confirmed: `auth.test OK`, team=Jarvis HQ, user=openclaw_jarvis). It works for posting notifications to channels. Full DM/channel-history sync requires the `xoxp-` user token.
