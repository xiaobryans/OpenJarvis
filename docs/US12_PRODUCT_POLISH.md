...
| Hotkey/voice settings UX | DONE | Speech & Hotkeys section: global hotkey (Cmd+K), system panel hotkey (Cmd+I), wake-word status (not available, workaround shown), STT toggle, microphone/TTS backend status | `frontend/src/pages/SettingsPage.tsx` |
...
| Wake-word activation | EXTERNALLY_NOT_PROVEN | Requires OpenWakeWord or compatible model installed separately | Workaround shown in UI: use Cmd+K or chat input |
