...
class TestActiveSprintUIUnification:
    def test_sk_roadmap_active_sprint_is_ui_unification(self, sk_client):
        ...
        assert "UI_UNIFICATION" in sprint or "FINAL_PHASE_A" in sprint or "PHASE_D" in sprint or "ONE_MEGA_SPRINT" in sprint or "GATE_CLEARANCE" in sprint or "CORE_OS" in sprint, (
            f"Expected UI unification, Final Phase A, Phase D, or final gate sprint, got: {sprint}"
        )

    def test_tower_active_sprint_is_ui_unification(self, tower_client):
        data = tower_client.get("/v1/control-tower/status").json()
        sprint = data["active_sprint"]
        assert "UI_UNIFICATION" in sprint or "CORRECTIVE" in sprint or "FINAL_PHASE_A" in sprint or "PHASE_D" in sprint or "ONE_MEGA_SPRINT" in sprint or "GATE_CLEARANCE" in sprint or "CORE_OS" in sprint, (
            f"Expected UI unification, corrective, Final Phase A, Phase D, or final gate sprint, got: {sprint}"
        )
...
class TestNeuralCommandCenterSprint:
    def test_active_sprint_is_neural_command_center(self, sk_client):
        data = sk_client.get("/v1/jarvis/roadmap").json()
        sprint = data["active_sprint"]
        assert "NEURAL" in sprint or "UI_UNIFICATION" in sprint or "FINAL_PHASE_A" in sprint or "PHASE_D" in sprint or "ONE_MEGA_SPRINT" in sprint or "GATE_CLEARANCE" in sprint or "CORE_OS" in sprint, (
            f"Expected neural command center, UI unification, Final Phase A, Phase D, or final gate sprint, got: {sprint}"
        )
