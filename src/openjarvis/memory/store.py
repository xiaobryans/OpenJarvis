...
class JarvisMemory:
    ...
    def write(
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            conn.commit()
        logger.debug(
            "Memory written: ns=%s project=%s entry_id=%s",
            namespace, project_id, entry.entry_id,
        )
        return entry
    ...
    def search(
        ...
        *,
        ...
        Filters by namespace and/or project_id if provided.
        Results ordered by recency (most recent first).
        """
        limit = max(1, min(limit, 200))
        query_lower = query.lower()

        clauses = ["(LOWER(content) LIKE ? OR LOWER(tags) LIKE ?)"]
        params: List[Any] = [f"%{query_lower}%", f"%{query_lower}%"]
...
__all__ = ["JarvisMemory", "MemoryEntry"]
